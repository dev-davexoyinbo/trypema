...
///
/// ```
/// use trypema::{RateLimit, RateLimiter, RateLimiterOptions, LocalRateLimiterOptions};
...
impl LocalRateLimiterProvider {
    ...
    ///
    /// ```
    /// # use trypema::{RateLimit, RateLimitDecision, RateLimiter, RateLimiterOptions, LocalRateLimiterOptions};
    /// # use trypema::{WindowSizeSeconds, RateGroupSizeMs, HardLimitFactor};
    /// # let rl = RateLimiter::new(RateLimiterOptions {
    /// #     local: LocalRateLimiterOptions {
    /// #         window_size_seconds: WindowSizeSeconds::try_from(60).unwrap(),
