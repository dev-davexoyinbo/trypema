...
fn test_local_cleanup_loop_runs() {
    // Create a rate limiter with local support
    let options = RateLimiterOptions {
        ...
        redis: crate::RedisRateLimiterOptions {
            connection_manager: {
                // We won't actually use this in the local-only test
...
fn test_cleanup_loop_keeps_active_entries() {
    let options = RateLimiterOptions {
        ...
        #[cfg(any(feature = "redis-tokio", feature = "redis-smol"))]
        redis: crate::RedisRateLimiterOptions {
            connection_manager: { panic!("should not reach here") },
            prefix: None,
            window_size_seconds: WindowSizeSeconds::try_from(1).unwrap(),
            rate_group_size_ms: RateGroupSizeMs::try_from(100).unwrap(),
            hard_limit_factor: HardLimitFactor::default(),
...
#[tokio::test]
async fn test_redis_cleanup_loop_with_tokio() {
    use crate::RedisKey;
